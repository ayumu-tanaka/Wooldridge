...
\item
  平行トレンド仮定（介入がなければ処置群・対照群の平均変化が同じ）の下で、交差項の係数
  \(\hat{\delta}_1\) が政策の因果的効果を表す。
\end{itemize}
\end{frame}
...
\item
  これにより時間不変の未観測効果 \(a_i\) は除去できるが、\(\Delta x_i\)
  と \(\Delta u_i\)
  が相関しない（差分後の外生性）ことが一致性の条件である。
\end{itemize}
\end{frame}
...
\item
  \textbf{1階差分法:} \(a_i\)
  を引き算で消去し、時間不変の未観測要因によるバイアスを軽減する（因果解釈には差分後の外生性など追加条件が必要）。
\end{itemize}
\end{frame}
